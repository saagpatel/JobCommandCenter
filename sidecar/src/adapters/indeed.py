...
class IndeedAdapter(BaseAdapter):
    _MAX_PER_SESSION = 15
    _MIN_INTERVAL_SECONDS = 120  # 2 minutes between submissions
        ...
        try:
            ...
            for selector in [
                ...
                if await btn.count() > 0:
                    await btn.first.wait_for(timeout=10_000)
            ...
            for step in range(max_steps):
                ...
                question_groups = page.locator(
                    'div[data-testid="ia-Questions-Item"], div.ia-FormFieldContainer, div[class*="ia-Question"]'
                )
                ...
                if q_count > 0:
                    ...
                    for i in range(q_count):
                        ...
                        label_el = group.locator("label, legend")
                ...
                submit_btn = page.locator(
                    'button[data-testid="ia-submitButton"], button:has-text("Submit your application"), button:has-text("Submit application")'
                )

                if await submit_btn.count() > 0:
                    if dry_run:
                        screenshot_path = await screenshot_on_failure(page, f"indeed_dry_run_{job.company}")
                        ...
                        confirmation = page.locator(
                            'h1:has-text("application was sent"), div:has-text("Your application has been submitted")'
                        )
                        try:
            ...
            # Exhausted steps without submitting
            if page and not page.is_closed():
                screenshot_path = await screenshot_on_failure(page, f"indeed_stuck_{job.company}")
                await page.close()
            ...
            if page and not page.is_closed():
                try:
                    await screenshot_on_failure(page, f"indeed_error_{job.company}")
                except Exception:
                    pass
                await page.close()
            return SubmissionResult(
